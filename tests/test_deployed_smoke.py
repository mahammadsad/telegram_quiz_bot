...
def test_synthetic_staging_init_data_is_short_lived_and_cryptographically_valid(
    ...
    signing_credential = "123456789:staging-smoke-test-only"
    init_data = build_synthetic_staging_init_data(signing_credential, auth_date=1_800_000_000)
    monkeypatch.setattr("telegram.auth.time.time", lambda: 1_800_000_001)

    user = verify_init_data(init_data, signing_credential, max_age_seconds=300)
