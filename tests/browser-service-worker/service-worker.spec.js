...
const ACTIVE_SHELL_CACHE = "quiz-miniapp-shell-v8.7.16-ui1";
...
test("answer-free projections enter the fallback cache only with the opt-in response header", async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    async function exercise(answerFree) {
      ...
      const firstResponse = await fetch(url, { cache: "no-store" });
      const first = await firstResponse.json();
      const cache = await caches.open("quiz-answer-free-v8.7.16-ui1");
...
test("shell requests refresh stale cached assets from the network", async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    const cache = await caches.open("quiz-miniapp-shell-v8.7.16-ui1");
    await cache.put(
      "/index.js",
  ...
  await Promise.all([
    ...
    page.evaluate(async () => {
      ...
      void navigator.serviceWorker.register(
        "/service-worker.js?upgrade-recovery=8.7.16-ui1",
