...
def setup_run(monkeypatch, valid_questions, existing_run=None):
    ...
    monkeypatch.setattr(bot.polls_repo, "delete_by_run_slot", lambda *args: events.append(("clear_deliveries", args)))
...
def test_save_export_and_generated_state_precede_telegram(monkeypatch, valid_questions):
    ...
    assert labels.index("save_pack") < labels.index("export") < labels.index("generated") < labels.index("telegram") < labels.index("posted")
...
def test_force_regenerate_uses_explicit_replacement_path(monkeypatch, valid_questions):
    ...
    deleted = []
    monkeypatch.setattr(bot.polls_repo, "delete_by_run_slot", lambda quiz_id, bot_type: deleted.append((quiz_id, bot_type)))
    result = bot.run_subject_quiz("history", target_date=date(2026, 7, 10), force_regenerate=True)
    assert result == "generated_and_posted"
    assert deleted == [("20260710-history", "mock_test")]
...
def test_invalid_repaired_json_is_never_accepted():
    class Pool:
        def __init__(self): self.calls = 0
        def generate_subject_quiz(self, **kwargs): self.calls += 1; return "still-bad", {}
    pool = Pool()
