-- Keep the durable chapter-level backlog, but spend each bounded worker claim
-- on the subjects with the smallest safe reserve first. The 150-question
-- reserve is the audited 15-day target at ten questions per daily quiz.
--
-- Capacity intentionally uses the exact fail-closed eligibility contract from
-- get_verified_question_inventory: verified question, verified source, and at
-- least one current verified supporting fact. Once all subjects reach the
-- reserve, the existing per-subject round-robin order continues unchanged.
...
) is 'Claims safe-reserve gaps first, then preserves bounded per-subject round-robin replenishment.';
...
-- Advance the fail-closed deployment contract. The predecessor keeps every
-- existing check; this wrapper adds an exact migration-ledger requirement for
-- the reserve-aware dispatcher.
alter function public.get_platform_contract_v1()
    rename to get_platform_contract_v1_before_reserve_priority;

revoke all on function public.get_platform_contract_v1_before_reserve_priority()
    from public, anon, authenticated;
...
begin
    v_base := public.get_platform_contract_v1_before_reserve_priority();
    ...
    if to_regclass('supabase_migrations.schema_migrations') is not null then
        execute $migration_check$
            select exists (
                ...
                where version = '20260904164836'
                   or name = 'reserve_aware_replenishment_claims'
            )
    ...
    return v_base || jsonb_build_object(
        ...
        'required_migration_version', '20260904164836',
