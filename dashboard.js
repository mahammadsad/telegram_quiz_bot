(function(){
  ...
  function renderNextAction(data,due,weakSubject){var copy=el("next-copy"),link=el("next-link"),plan=data.studyPlan||{},target=plan.questionTarget||0;if(plan.version===1){if(plan.nextAction==="continue_due_revision"){copy.textContent=(plan.personalized?"আপনার পছন্দ ও পুনরাবৃত্তির সময়সূচি অনুযায়ী ":"আজকের জন্য ")+bn(target||due)+"টি প্রশ্ন আগে ঝালিয়ে নিন।";link.href="practice.html?source=due"+(plan.subjectKey?"&subject="+encodeURIComponent(plan.subjectKey):"");link.textContent="পুনরাবৃত্তি শুরু করুন";return}if(plan.nextAction==="practice_weak_topics"){copy.textContent="আপনার ফল ও বিষয়-পছন্দ অনুযায়ী "+bn(target)+"টি দুর্বল প্রশ্ন অনুশীলন করুন।";link.href=plan.subjectKey?"practice.html?source=weak_topic&subject="+encodeURIComponent(plan.subjectKey):"practice.html?source=wrong";link.textContent="দুর্বল টপিক অনুশীলন";return}if(plan.nextAction==="target_exam_mock"&&plan.examKey){copy.textContent="আপনার টার্গেট পরীক্ষা অনুযায়ী একটি প্রকাশিত মক বেছে নিন।";link.href="mock.html?exam="+encodeURIComponent(plan.examKey);link.textContent="টার্গেট মক দেখুন";return}if(plan.nextAction==="preferred_subject_quiz"&&plan.subjectKey){copy.textContent="আপনার পছন্দের বিষয় থেকে আজকের বাকি "+bn(plan.remainingQuestions||target)+"টি প্রশ্ন শুরু করুন।";link.href="./?subject="+encodeURIComponent(plan.subjectKey);link.textContent="পছন্দের বিষয় খুলুন";return}if(plan.nextAction==="goal_complete"){copy.textContent="আজকের প্রশ্ন-লক্ষ্য পূর্ণ হয়েছে। চাইলে মক পরীক্ষা বা পুনরাবৃত্তি চালিয়ে যান।";link.href="mock.html";link.textContent="মক পরীক্ষা দেখুন";return}}if((data.overdueQuestions||0)>0){copy.textContent=bn(data.overdueQuestions)+"টি মেয়াদ-পেরোনো প্রশ্ন আগে ঝালিয়ে নিন।";link.href="practice.html?source=due";link.textContent="পুনরাবৃত্তি শুরু করুন"}else if(due>0){copy.textContent="আজকের জন্য "+bn(due)+"টি প্রশ্ন পুনরাবৃত্তি বাকি।";link.href="practice.html?source=due";link.textContent="পুনরাবৃত্তি শুরু করুন"}else if((data.weakQuestions||0)>0){copy.textContent="দুর্বল টপিক থেকে কয়েকটি প্রশ্ন অনুশীলন করুন।";link.href=weakSubject?"practice.html?source=weak_topic&subject="+encodeURIComponent(weakSubject):"practice.html?source=wrong";link.textContent="দুর্বল টপিক অনুশীলন"}else{copy.textContent="আজকের লক্ষ্য ধরে রাখতে একটি কুইজ দিন।";link.href=quizHomeUrl;link.textContent="কুইজ খুলুন"}}
