...
This runbook is the production gate for application version `8.7.9`. Passing local unit tests is not production evidence.
...
Release 8.7.9 has no new database migrations. Its rollback target is release
`082dc01aca7cff6f11a8b352045de9af725997e9` (8.7.8), which supports the current
...
2. Verify `/version` reports `applicationVersion: 8.7.9`, the expected full `commitSha`, staging environment and build time.
