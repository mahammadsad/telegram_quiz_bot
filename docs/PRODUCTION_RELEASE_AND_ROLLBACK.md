...
This runbook is the production gate for application version `8.7.5`. Passing local unit tests is not production evidence.
...
2. Verify `/version` reports `applicationVersion: 8.7.5`, the expected full `commitSha`, staging environment and build time.
