...
This runbook is the production gate for application version `8.7.8`. Passing local unit tests is not production evidence.
...
Release 8.7.8 has no new database migrations. Its rollback target is release
`019fd21341afa630cdb7603975764274f595f66c` (8.7.7), which supports the current
...
2. Verify `/version` reports `applicationVersion: 8.7.8`, the expected full `commitSha`, staging environment and build time.
