...
def submit_practice(
    *,
    ...
    try:
        result = get_client().rpc(
            "submit_practice_question_report",
            {
                "p_question_id": question_id,
                "p_user_id": user_id,
                "p_client_attempt_id": client_attempt_id,
        ...
        safe_errors = (
            "already reported",
            "report rate limit exceeded",
            "not linked to this completed revision attempt",
        )
        for safe_error in safe_errors:
            if safe_error in message:
                raise ValueError(safe_error) from exc
        raise
    if not isinstance(result.data, dict) or result.data.get("status") != "accepted":
        raise DatabaseIntegrityError("Practice question report RPC returned an invalid result.")
    return result.data
