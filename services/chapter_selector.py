...
def select_chapter(subject_key: str, target_date: date, history: list[dict] | None = None) -> str:
    ...
    normalized_history = [
        (str(row.get("chapter") or ""), _as_date(row.get("selected_for")))
        for row in history
        if row.get("chapter") and _as_date(row.get("selected_for"))
    ]
    ...
    previous_dates = {chapter: used_on for chapter, used_on in normalized_history}
